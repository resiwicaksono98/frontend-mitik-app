...
const ManageAdmin = () => {
  ...
  const handleDelete = async ({ id }) => {
    try {
      await authRequest.delete(`/admins/${id}`);
          ...
          {admins.map((admin, i) => (
            <tr
              className={`text-center  ${
                admin.role === "superAdmin" && "hidden"
              }`}
              key={i}
            >
              <td className="border border-slate-300">{i - 1 + 1}</td>
              <td className="border border-slate-300">{admin.name}</td>
              <td className="border border-slate-300">{admin.email}</td>
              <td className="border border-slate-300">{admin.phone_number}</td>
              <td className="border border-slate-300">{admin.role}</td>
              <td className="grid grid-cols-1 gap-2 py-2 px-2 text-white">
                <button
                  className={` bg-red-500 hover:bg-red-700 rounded-lg py-2`}
                  onClick={() => handleDelete({ id: admin.id })}
                >
                  Delete
                </button>
              </td>
